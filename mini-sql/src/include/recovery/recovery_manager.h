...
class RecoveryManager {
 public:
    ...
    for (; iter != log_recs_.end(); ++iter) {
      ...
      switch (log_rec.type_) {
        ...
        case LogRecType::kInsert:
          handleInsert(log_rec);
          break;
        case LogRecType::kDelete:
          handleDelete(log_rec);
          break;
        case LogRecType::kUpdate:
          handleUpdate(log_rec);
          break;
        case LogRecType::kBegin:
          break;
        case LogRecType::kCommit:
          handleCommit(log_rec);
          break;
  ...
  void handleInsert(const LogRec& log_rec) {
    data_[log_rec.ins_key_] = log_rec.ins_val_;
  }

  void handleDelete(const LogRec& log_rec) {
    data_.erase(log_rec.del_key_);
  }

  void handleUpdate(const LogRec& log_rec) {
    data_.erase(log_rec.old_key_);
    data_[log_rec.new_key_] = log_rec.new_val_;
  }

  void handleCommit(const LogRec& log_rec) {
    active_txns_.erase(log_rec.txn_id_);
  }

  void handleAbort(const LogRec& log_rec) {
  ...
  void UndoPhase() {
    for (const auto &txn_pair : active_txns_) {
      auto txn_id = txn_pair.first;
      lsn_t lsn = txn_pair.second;
